package com.example.smartlenskotlin.tf.mobile
...
class TfMobileDetector @Inject constructor(val mContext: Context) : TfDetector {
    companion object {
        val INPUT_SIZE           = 224L

        private val THRESHOLD    = 0.1f
        private val MAX_RESULTS  = 3
        private val MODEL_FILE   = "mobilenet_v1.pb"
    ...
    private fun initLabels(): Boolean {
        ...
        try {
            val stream = mContext.assets.open(LABEL_FILE)
            val br: BufferedReader = BufferedReader(InputStreamReader(stream))
    ...
    private fun recognizeImage(imageFloats: FloatArray): List<TfRecognition> {
        ...
        mTfInterface?.let {
            ...
            val pq = PriorityQueue<TfRecognition>(
                3,
                Comparator<TfRecognition> { lhs, rhs -> Float.compare(rhs.confidence!!, lhs.confidence!!) })
